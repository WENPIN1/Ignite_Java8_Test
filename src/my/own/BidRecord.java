...
//		@QuerySqlField.Group(name = "bid_idx", order = 20)})
	...
	@QuerySqlField(orderedGroups={
		@QuerySqlField.Group(name = "bid_idx", order = 7)})
	private Double win;
	
	@QuerySqlField(orderedGroups={
		@QuerySqlField.Group(name = "bid_idx", order = 8)})
	...
	@QuerySqlField(orderedGroups={
		@QuerySqlField.Group(name = "bid_idx", order = 1, descending=true)})
	private Double amount;
	
	@QuerySqlField(orderedGroups={
		@QuerySqlField.Group(name = "bid_idx", order = 2)})
	private Double limitWin;
	
	@QuerySqlField(orderedGroups={
		@QuerySqlField.Group(name = "bid_idx", order = 3)})
	private Double limitPlace;
	
	@QuerySqlField(index=false, orderedGroups={
		@QuerySqlField.Group(name = "bid_idx", order = 9)})
	...
	@QuerySqlField(index=false, orderedGroups={
		@QuerySqlField.Group(name = "bid_idx", order = 4)})
	private Long dateA;
	 ...
	 * @param dateA
	 */
	public BidRecord(Long bid, int race, int horse, double win, double place, double amount, double limitWin,
		double limitPlace, String uid_a, String currency, String raceType, String raceDate, double sgWin,
		double sgPlace, Long dateA, int live) {
		super();
		this.bid = bid;
		this.race = race;
		this.horse = horse;
	...
	/*public void settleData(double win, double place) {
		this.win = win;
		this.place = place;
		this.dateA = System.currentTimeMillis();
		if (win ==0 || place==0) {
			System.out.println("win+place:" + win + "," +place);
			Thread.dumpStack();
		}
